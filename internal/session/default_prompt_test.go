...
func TestDefaultSystemPrompt_ContainsThinkTags(t *testing.T) {
	...
	for _, want := range []string{"DO the task", "For large writes", "Split file creation/rewrites", "read_file", "why", "Only change files", "background=true", "timeout_sec", "Never use emojis"} {
