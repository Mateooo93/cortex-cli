...
func renderThinkingText(text string, s Styles, width int) string {
	...
	for i, line := range lines {
		...
		for j, wl := range wrapped {
			sb.WriteString("  ")
			// Add a small thinking icon on the first line so the
			// user can visually distinguish thinking from regular
			// assistant text at a glance.
			if i == 0 && j == 0 {
				sb.WriteString(s.ThinkingStyle.Render("💭 "))
			}
...
// width is the inner content width, used for side-by-side diff rendering.
// When showThinking is false, persisted thinking messages are omitted from
// the output so the user does not see stale thinking blocks in the chat
// history after toggling the setting off.
func buildRenderedChat(messages []ChatMessage, s Styles, width int, showThinking bool) string {
	...
	for i, msg := range grouped {
		...
		// Drop persisted thinking messages when the user has disabled
		// extended-thinking display. The live streaming preview is
		// gated separately at the call sites in model.go.
		if !showThinking && msg.Type == MsgThinking {
			continue
		}
...
// When showThinking is false, persisted thinking messages are skipped so the
// returned line indices stay in sync with buildRenderedChat's filtered output.
func turnSeparatorInfos(messages []ChatMessage, s Styles, width int, showThinking bool) []TurnSepInfo {
	grouped := groupFileOperations(messages)
	...
	for i, msg := range grouped {
		...
		// Mirror buildRenderedChat's filter so line indices line up with
		// what the renderer actually paints to the screen.
		if !showThinking && msg.Type == MsgThinking {
			continue
		}
