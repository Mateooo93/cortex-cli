...
cortex-cli is an AI coding agent written in Go. It is a fork of
[vix](https://github.com/get-vix/vix) with cortex-specific changes:
an in-process session (no `vixd` daemon), a cortex-aware provider
layer, and a multi-agent swarm orchestrator. The TUI client is
unchanged from vix (same bubbletea + lipgloss + glamour stack).
